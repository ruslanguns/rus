...
import { AccountUpdateProfileInput, ApolloAngularSDK, User } from '@rus/web/core/data-access'
...
export class AccountProfileStore extends ComponentStore<AccountProfileState> {
  ...
  readonly updateProfileEffect = this.effect<AccountUpdateProfileInput>((input$) =>
    input$.pipe(
      mergeMap((input) =>
        this.sdk.accountUpdateProfile({ input }).pipe(
          tapResponse(
            ({ errors }) => {
  ...
  readonly updateUsernameEffect = this.effect<string>((username$) =>
    username$.pipe(
      mergeMap((username) =>
        this.sdk.accountUpdateUsername({ username }).pipe(
          tapResponse(
            ({ errors }) => {
              this.patchState({ errors })
              this.loadProfileEffect()
  ...
  readonly loadProfileEffect = this.effect(($) => {
    return $.pipe(
      ...
      switchMap(() =>
        this.sdk.accountProfile({}, { fetchPolicy: 'no-cache' }).pipe(
          tapResponse(
            ({ data, errors }) => {
              this.patchState({
